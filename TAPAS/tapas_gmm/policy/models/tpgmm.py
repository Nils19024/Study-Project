...
import riepybdlib.angular_representations as rbd_ar
import torch
...
class TPGMM:
    ...
    def patch_frame_transforms(
        ...
        if not position_only:
            ...
            b_quat = rbd_ar.Quaternion.from_nparray(quat_pose)
        ...
        if with_action_dim:
            ...
            b_pvel = (
                rbd_ar.Quaternion.from_nparray(quat_vel)
                if action_as_orientation
                else b_vel
            )
        ...
        if with_action_dim and not position_only:
            ...
            # )
            b_rvel = rbd_ar.Quaternion.from_nparray(quat_vel)  #  b_pvel
